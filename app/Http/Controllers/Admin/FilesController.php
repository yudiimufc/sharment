...
class FilesController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request('show_deleted') == 1) {
            ...
            $files = File::all();
            // $files = DB::table('file')
            // ->leftJoin('folder', 'file.folder_id', '=', 'folder.id')
            // ->select('file.*','folder.name')
            // ->get();

            // $files = File::join('folders', 'files.folder_id', '=', 'folders.id')
            //  ->select('files.*', 'folders.name AS folders_name')
            //  ->get();
        }
        $user = Auth::getUser();
        $userFilesCount = File::where('files.created_by_id', $user->id)->count();
